...
\def\thefootnote{}\footnotetext{$^{*}$Equal contribution. Code available at: \url{https://github.com/itc-hackathon/itc-hackathon}.}\def\thefootnote{\arabic{footnote}}
...
\begin{equation}
\begin{aligned}
\min_{\phi}\;\; &\mathbb{E}_{(c,\mathcal{D}_c)\sim\mathcal{D}}\; \mathbb{E}_{(x,y)\sim\mathcal{D}_c} \\[2pt]
&\mathrm{KL}\big(p_\theta(y|x,c)\,\|\,p_{\theta+H_\phi(c)}(y|x)\big).
\end{aligned}
\label{eq:meta}
\end{equation}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/scaling_1.png}
\caption{Repeated compaction scales recall past \DTL{}'s single-encode limit: recall vs.\ conversation length. \mt{scripts/scaling\_sweep.py}.}
\label{fig:scaling}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/scaling_2.png}
\caption{Query-time context (log scale), \naplora{} vs.\ the text-RAG ablation, for the same growing conversation as Figure~\ref{fig:scaling}. \mt{scripts/scaling\_sweep.py}.}
\label{fig:scalingcost}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/large_scale_1.png}
\caption{Statistical validation -- recall, mean $\pm$ std over $5$ seeds $\times$ $20$ facts $\times$ $5$ haystack sizes ($n{=}100$/cell). \mt{scripts/large\_scale\_eval.py}.}
\label{fig:largescale}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/large_scale_2.png}
\caption{Query-time context (log scale) for the same sweep as Figure~\ref{fig:largescale}, including the trained Cartridges baseline (\S\ref{sec:cartridges}). \mt{scripts/large\_scale\_eval.py}.}
\label{fig:largescalecost}
\end{figure}
