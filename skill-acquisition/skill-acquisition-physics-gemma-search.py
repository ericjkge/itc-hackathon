"""Search for new physics "golden examples" calibrated to gemma-2b specifically.

golden-examples.txt (the existing 5-question battery) was vetted against
Qwen3-4B-Instruct-2507 ("MAKE SURE QWEN-4B GREEDY TURNED ON") via the
in_context/adapter conditions only -- it was never tested cold, and never
against gemma-2-2b-it. AgentHN's live webapp runs the gemma_demo checkpoint
(gemma-2-2b-it), a much smaller model, so its base/in-context/adapter
behavior on these formulas differs.

This script tests a larger bank of CANDIDATE physics word problems against
gemma_demo specifically, under three conditions (base / in_context /
adapter), and prints the subset where:
    base    = WRONG   (cold model can't do it without help)
    adapter = RIGHT   (internalizing the doc into a LoRA fixes it)
(in_context may be either -- that's not part of the filter).

Run:
    /home/ubuntu/doc-to-lora/.venv/bin/python scripts/skill-acquisition-physics-gemma-search.py
"""
...
# New candidates -- distinct numbers/formulas from the existing 5 golden
# examples, spanning every formula in DOC, to search for ones where gemma-2b
# specifically fails cold but succeeds once the doc is internalized.
CANDIDATES = [
    dict(
...
def fmt_question_dict(q):
    return (
        "    dict(\n"
        f"        question={q['question']!r},\n"
        f"        expected={q['expected']!r},\n"
        f"        formula={q['formula']!r},\n"
        "    ),"
    )
...
def main():
    ...
    rows = []
    qualifying = []
    for q, base, ic, ad in zip(CANDIDATES, base_answers, in_context_answers, adapter_answers):
        base_pass = check_numeric(base, q["expected"])
        ic_pass = check_numeric(ic, q["expected"])
        ad_pass = check_numeric(ad, q["expected"])
        rows.append(dict(question=q["question"], formula=q["formula"], expected=q["expected"],
                          base_answer=base, base_pass=base_pass,
                          in_context_answer=ic, in_context_pass=ic_pass,
                          adapter_answer=ad, adapter_pass=ad_pass))
        print(f"{'=' * 80}\n{q['question']}  [{q['formula']}]\nExpected: {q['expected']:g}\n{'=' * 80}")
        print(f"--- base      ({'PASS' if base_pass else 'FAIL'}) --- {base}")
        print(f"--- in_context({'PASS' if ic_pass else 'FAIL'}) --- {ic}")
        print(f"--- adapter   ({'PASS' if ad_pass else 'FAIL'}) --- {ad}\n")
        if (not base_pass) and ad_pass:
            qualifying.append(q)
    ...
    print(f"\nQualifying (base=WRONG, adapter=RIGHT): {len(qualifying)}/{n}\n")
    for q in qualifying:
        print(fmt_question_dict(q))

    output_path = args.output or f"{args.d2l_dir}/skill_acquisition_physics_gemma_search_{time.strftime('%Y%m%d_%H%M%S')}.json"
    with open(output_path, "w") as f:
        json.dump({"doc": DOC, "rows": rows, "qualifying": qualifying}, f, indent=2)
